...
The blogdown/Hugo setup is fairly flexible and powerful, but sometimes more complex than what is needed. At least I find this to be the case. While I'm using Hugo/blogdown for my [personal website](https://www.andreashandel.com) I have been using a basic RMarkdown/Github setup for my online courses, such as [this one](https://andreashandel.github.io/IDEMAcourse/).

I previously described how to set up a simple RMarkdown/Github website [in this blog post](/post/github-website/). 

While this setup is easy and quick, it's not that easy to adjust things. This is where `distill` comes in. A while ago, the folks at RStudio developed the [`distill` package](https://rstudio.github.io/distill/), which basically lets you write RMarkdown documents, with some additional features.

More recently, the `distill` framework has been increasingly used by people who want some ability to configure their website, but who find the blogdown/Hugo setup too complex for their needs. This will likely see increasing development and provides a nice middle ground with regard to complexity and flexibility between the most basic RMarkdown/Github website and the full blogdown/Hugo setup.  

In this post, I show you how to create your own website using this setup.
...
There is a nice website that collects examples of `distill` websites, cleverly named [the distillery](https://jhelvy.github.io/distillery/).
...
### The file `index.Rmd` 
This is the main landing page of your website. It always needs to be there and you fill it with the content you want on your main page. It should have opened in RStudio, if not, click on it to open it. You will see some stuff at the top between the `---` symbols, that's called the YAML header and is used for formatting. Below that it's pretty much a regular R Markdown document, you can place R code and Markdown in ther. If you are new to R Markdown, you can learn more about it [on the R Markdown website](https://rmarkdown.rstudio.com/). I also have a discussion of R Markdown and reproducibility [on one of my course pages](https://andreashandel.github.io/MADAcourse/1e_ToolsforReproducibility.html). You'll find additional links to potentially useful R Markdown (and Github) resources there.

### The file `about.Rmd` 
This is another R Markdown file with so far little content. 

### The file `_site.yml` 
This short file contains the main settings for your site that control the overall look. It also defines the structure, i.e. the menu bar at the top of your site. We'll edit this file shortly.    
...
Let's create some content. Open the `index.Rmd` file and write some introductory text at the bottom of the file. Something like _This is my new website, it's going to be awesome!_ or whatever you like to write.

Then open the `about.Rmd` file and also write some text, e.g. something like _On this website you can find all the cool materials I am producing._.

Finally, let's create yet another file. Go to `File`, `New File`, `R Markdown` choose as output format `HTML document` and give it a title and name. Let's call it _About me_. A template will open. Delete the `output: html_document` line in the YAML since our layout is controlled by `_site.yml` and `index.Rmd`. Also delete the existing content (or keep it at the bottom if you want to see how it looks when compiled). Then write text introducing yourselves. Add a picture using either [the markdown syntax]() or the [knitr syntax](). Save this new page as `aboutme.Rmd` into the main folder of your website.

Note that another option, which I usually use, is to go into the folder, make a copy of an existing file and rename. E.g. copy `project1.Rmd` to `project2.Rmd`, then open and edit. Either approach works.
...
**Warning! Any YAML file or YAML part of a file is very picky about the exact formatting, and the number of leading empty spaces. Often, if you change something in those files and the site doesn't compile, it means you forgot to add the required empty spaces or added more than you should. Always check first if the spacing is correct.** 

Here, make sure your new `navbar` entries is exactly positioned like the entries above it.

Once created, save all your files, then rebuild the website. If things work, the preview window should show up, and you should now see the text you wrote and be able to click on your new `About Me` entry. (You'll find the newly created html files in the `docs` folder.)
...
Emi Tanaka (who I do not know personally) is one of the sites featured on [the distillery](). Her website [is here](https://emitanaka.org/). 
As of the time of this writing, she has a theme with different fonts and colors. The theme is controlled by a CSS file for her site. You can get it by [clicking here](https://github.com/emitanaka/emitanaka.github.io/blob/master/assets/theme.css) and downloading it into the main directory of your new website.

After you have done that, open the `_site.yml` file and `theme: theme.css` as a new line under the `title` line. Save, rebuild your website, and you should see it looking differently.

Since the promise was that we'll get a website going in 30 minutes, we won't do any more styling now 😁. As you start customizing your look more, you might want to start read the section [Creating a Website](https://rstudio.github.io/distill/website.html) on the distill website.
...
* Open GitKraken, go to `File -> Init Repo -> Local Only`. Give it the name of your main website directory, e.g. `mywebsite` and the `Initialize In` folder should be the folder above where you created the website, such that the `Full path` entry is the actual location of your website. For `.gitignore Template` you can choose `R`. 

<img src="gitkraken.png" width="100%" style="display: block; margin: auto;" />


Once done, click `Create repository`. You should see a bunch of files ready for staging on the left. Click `Stage all changes` enter a commit message, commit. Then Click the `Push` button. You'll get a message about no remote existing and if you want to add one. Say `yes`. A menu on the left should show up. Make sure the repository name is the same as your folder name. Then click the green button. At this point, if you didn't properly connect GitKraken and GitHub previously, you'll likely get an error message. Follow the error message and the [connect Gitkraken to your Github account](https://www.gitkraken.com/resources/gitkraken-github-cheat-sheet) information to get it to work.

## Github website

For the last step, go to your account on Github.com and find the repository for the website you just created. On the bar at the top, in the right corner there should be the `Settings` button. Click on it. Scroll down until you find the `GitHub Pages` section. Under `Source`, select `Main` and then choose `/docs` as the folder. Don't choose a theme since we are using our own. Save those changes. Now if you look right underneath the `GitHub Pages` section, there should be something like `Your site is ready to be published at https://andreashandel.github.io/mywebsite/`. Click on the link. If everything works (it could take a minute or so), your website is now live and public! 
...
The whole area of _use R Markdown & Co to make websites_ is still under rapid development. Here are a few sources that - as of this writing - might be useful and not too outdated.
