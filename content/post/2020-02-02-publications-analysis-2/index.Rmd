...
The `bibliometrix` package doesn't suffer from the problems that I encountered in [part 1](/post/publication-analysis-1/) of this post when I tried  the `scholar` package (and Google Scholar) have. The downside is that I can't get some of the information, e.g. my annual citations. So it seems there is not (yet) a comprehensive solution, and using both packages seems best. 
