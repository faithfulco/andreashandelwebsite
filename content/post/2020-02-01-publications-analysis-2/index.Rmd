...
Instead of doing things by hand, there are nicer/faster ways using R. in [part 1](/post/publication-analysis-1/), I did a few things using the `scholar` package. 
...
Ok that's not too bad. It couldn't find the Lancet Global Health, Current Opinion Systems Biology does indeed not have an impact factor, and PNAS is clearly wrong. The others seem reasonable. But since I don't know what year those IF are from, and if the rest is fully reliable, I would take this with a grain of salt.

# Discussion

The `bibliometrix` package doesn't suffer from the problems mentioned above that the `scholar` package (and Google Scholar) have. The downside is that I can't get some of the information, e.g. my annual citations. So it seems there is not (yet) a comprehensive solution, and using both packages seems best. 
