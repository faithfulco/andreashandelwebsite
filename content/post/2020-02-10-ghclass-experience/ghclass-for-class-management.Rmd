...
Some exercises involved having students go into each other repositories and provide feedback through Issues, which the repo owner was asked to address and then close the Issue. I didn't want to go through each repo and check/read the issue. I reached out to Colin Rundel, the `ghclass` author, and asked if there was a way to get stats on repositories. He was able to implement something. 

Unfortunately, at the time of this writing, this is not yet on CRAN or the master branch on GitHub. It's on a side branch, which you can install like this:
...
```
#rs <- ghclass::org_repo_stats(orgname,  filter_type = "in:user")  
```
...
<!-- #new function in development branch, pulls all kinds of stats for each repo -->
<!-- ```{r repo-stats-1} -->
...
<!-- #new function in development branch -->
<!-- #get contributors for repos -->
<!-- r1 <- org_repos(orgname,filter = "Coding") -->
<!-- r2 <- org_repos(orgname,filter = "coding") -->
